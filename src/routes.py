    ...
    def evidence_collect(req: EvidenceReq):
        ...
        return {"evidence": ev, "audit_id": audit_id, "meaning":"Collected deploy evidence for the agent to reason on."}

    # --- 5) metrics ingest ---
    @router.post("/v1/metrics/ingest")
    def metrics_ingest(req: MetricsIngestReq):
        audit_id = write({"type":"metrics.ingest", **req.model_dump()})
        return {"ok": True, "audit_id": audit_id, "meaning":"Captured a metrics snapshot used for risk scoring."}

    # --- 6) policy eval ---
    @router.post("/v1/policy/eval")
    def policy_eval(req: PolicyEvalReq):
        allowed, denies = eval_policies(req.env, req.manifests, req.rules)
        audit_id = write({"type":"policy.eval", "env": req.env, "allowed": allowed, "deny_reasons": denies})
        return {"allowed": allowed, "deny_reasons": denies, "audit_id": audit_id, "meaning":"Policy guardrails for the target environment."}
    ...
    def evaluate(req: EvaluateReq):
        ...
        req_dict["manifests_raw"] = [{"kind": m.kind, "metadata": {"name": m.name, "namespace": m.namespace}} for m in req.manifests]
    ...
    def plan_only(req: EvaluateReq):
        ...
        audit_id = write({"type":"plan", "decision": d["decision"], "plan": p, "env": req.env, "service": req.service})
        return {"plan": p, "audit_id": audit_id, "meaning":"Returns a rollout plan (canary/blue-green/rolling) matched to risk."}

    # --- 9) approvals ---
    @router.post("/v1/approve")
    def approve(req: ApprovalReq):
        set_decision(req.audit_id, req.approver, req.decision)
        audit_id = write({"type":"approval", "audit_id": req.audit_id, "approver": req.approver, "decision": req.decision})
        return {"ok": True, "audit_id": audit_id, "meaning":"Records a human approval/denial for high-risk changes."}
    ...
    def decision_lookup(audit_id: str):
        ...
        return {"audit_id": audit_id, "approval": d, "meaning":"Fetches the human gate decision used to proceed/rollback."}

    # --- 11) audit tail ---
    @router.get("/v1/audit/tail")
    def audit_tail():
        return {"events": read_last(100), "meaning":"Last 100 audit events (evidence, policy, evaluate, approvals)."}

    # --- 12) ChatOps ask (Anthropic everywhere) ---
    @router.post("/v1/chatops/ask")
    async def chatops_ask(req: AskReq):
        llm = AnthropicLLM()
        schema_hint = {"answer":"(string)","actions":["(optional list of recommended next actions)"],"confidence":0.7}
...
Return JSON with keys: answer, actions, confidence."""
        ...
        return {"result": out, "audit_id": audit_id, "meaning":"Anthropic-assisted guidance for deploy decisions and next steps."}
